...
const Header = () => {
  ...
  const mainNavigation = [
    ...
    { key: "davanu", name: "Dāvanu kartes", href: "/lv/davanu-kartes/" },
  ];
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-neutral-200">
                ...
                {mainNavigation.map((item) =>
                  item.hasDropdown ? (
                    ...
                    <li key={item.key} className="shrink-0">
                      <a href={item.href} className="text-[15px] text-text/90 hover:text-[hsl(var(--primary))] font-medium whitespace-nowrap link-underline">
                        {item.name}
                      </a>
                    </li>
                  )
      ...
      <div className="md:hidden container mx-auto max-w-7xl px-4 py-2">
        <div className="flex items-center justify-between">
          ...
          <button className="p-2 text-text/80" aria-label="Open menu" aria-expanded={mobileOpen} onClick={toggleMobile}>
            {mobileOpen ? (
      ...
      >
        ...
        <div className="h-[calc(100vh-56px)] overflow-y-auto">
          {/* === MOBILE UTILITY (Languages, Contacts, Sponsor) === */}
          <div className="px-4 pt-4 pb-2 space-y-4">
            {/* Languages */}
            <div>
              <div className="text-[10px] tracking-wide text-text/60 font-semibold uppercase mb-1.5">Valodas</div>
                ...
                {languages.map((l, i) => (
                  <span key={l.code} className="flex items-center">
                    {l.active ? (
                      ...
                      <a href={l.href} className="text-text/80 hover:text-[hsl(var(--primary))] link-underline">
                        {l.code}
                      </a>
                    )}
            ...
            {/* Contacts */}
            <div>
              <div className="text-[10px] tracking-wide text-text/60 font-semibold uppercase mb-1.5">Kontakti</div>
              <div className="flex flex-col gap-1.5 text-sm">
                <a href="tel:+37128677177" className="flex items-center gap-2 text-text/90 link-underline">
                ...
                <a href="mailto:info@latvianhorses.lv" className="flex items-center gap-2 text-text/90 link-underline">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
                          d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
                  </svg>
            ...
            {/* Sponsor card */}
            <div>
              <div className="text-[10px] tracking-wide text-text/60 font-semibold uppercase mb-1.5">Galvenais sponsors</div>
              <div className="flex items-center gap-3 border border-neutral-200 rounded-xl p-3">
                <div className="w-14 h-10 rounded-lg border border-neutral-200 grid place-items-center text-[10px] text-text/60">
                  LOGO
                </div>
                <div className="min-w-0">
                  <div className="text-sm font-semibold leading-tight">Main Sponsor</div>
                  <div className="text-xs text-text/70 leading-tight">Atbalsta Latvian Horses</div>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* === NAVIGATION === */}
          <nav className="mt-2 divide-y divide-neutral-200">
            {mainNavigation.map((item) =>
              item.hasDropdown ? (
                <div key={item.key} className="py-1">
                  ...
                  <div className={`grid transition-[grid-template-rows] duration-300 ease-in-out px-2 ${
                ...
                <a
                  ...
                  href={item.href}
                  onClick={() => setMobileOpen(false)}
                  className="block px-4 py-3 text-[15px] font-medium text-text/90 hover:text-[hsl(var(--primary))] whitespace-nowrap link-underline"
                >
                  {item.name}
                </a>
              )
