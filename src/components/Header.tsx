  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-neutral-200">
      <div className="w-full">
        ...
        <div className="border-b border-neutral-200">
          <div className="container mx-auto max-w-7xl px-4">
            <div className="flex items-center justify-between py-1.5">
              ...
              {/* Contacts inline */}
              <div className="flex items-center gap-5 text-xs text-text/80">
        ...
        {/* ===== Main bar: logo + centered nav + CTA ===== */}
        <div className="container mx-auto max-w-7xl px-4">
          <div className="grid grid-cols-3 items-center py-4 md:py-5">
            {/* Left: Logo */}
            <div className="flex items-center">
              <a href="/" className="text-lg md:text-xl font-semibold text-[var(--primary)] tracking-tight leading-none">
                Latvian Horses
              </a>
            </div>

            {/* Center: Main navigation (compact, with dropdowns) */}
            <nav className="hidden lg:flex justify-center">
              <ul className="flex items-center gap-6">
                {/* CTA-like Pasākumi отдельной ссылкой слева/справа от основного списка можно тоже показать, но
                   оставим его как отдельную кнопку справа, как в скрине */}
                {mainNavigation.map((item) =>
                  item.hasDropdown ? (
                    <li key={item.name} className="relative group">
                      <button className="text-sm text-text/90 hover:text-[var(--primary)] font-medium flex items-center gap-1">
                        {item.name}
                    ...
                    <li key={item.name}>
                      <a
                        href={item.href}
                        className="relative text-sm text-text/90 hover:text-[var(--primary)] font-medium after:block after:h-[2px] after:bg-transparent after:w-0 hover:after:bg-[var(--primary)] hover:after:w-full after:transition-all after:duration-200"
                      >
                        {item.name}
                      </a>
                    </li>
            ...
            <div className="flex justify-end">
              <a
                href="/lv/pasakumi/"
                className="inline-flex items-center rounded-md px-3.5 py-2 text-xs md:text-sm font-semibold text-white bg-[var(--primary)] hover:bg-[var(--primary-700)] transition-colors"
              >
                Pasākumi
              </a>
            </div>
