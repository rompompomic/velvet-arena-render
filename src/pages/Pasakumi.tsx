...
import { Calendar as CalendarIcon, List, MapPin, Clock, Users, ChevronRight } from "lucide-react";
...
const Pasakumi = () => {
  ...
  const events: EventItem[] = [{
    ...
    description: "Gada lielākā jauniešu sacensība Inčukalna rajonā ar starptautisku dalību.",
    image: youthCup,
    ...
    description: "Krāšņs svētku pasākums visiem zirgu mīļotājiem ar izrādēm un aktivitātēm.",
    ...
    description: "Starptautiskās šķēršļu pārvarēšanas sacensības ar augstu līmeni.",
    ...
    description: "Tradicionālās pavasara sacensības iekštelpu arēnā visiem līmeņiem.",
    image: eventCompetition1,
  ...
  const eventDates: Date[] = useMemo(() => events.map(e => new Date(e.date.getFullYear(), e.date.getMonth(), e.date.getDate())), [events]);
  ...
  const dayFiltered = filteredEvents.filter(e => !selectedDate || isSameDay(e.date, selectedDate));
          ...
          <div className="container mx-auto px-4 max-w-7xl">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-8">
              ...
              <div className="flex items-center bg-muted rounded-lg p-1">
                <button onClick={() => setViewMode("calendar")} className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors ${viewMode === "calendar" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>
                  <CalendarIcon className="w-4 h-4" />
                  Kalendārs
                </button>
                <button onClick={() => setViewMode("list")} className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors ${viewMode === "list" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}>
              ...
              <div className="flex flex-wrap gap-2">
                {categories.map(c => <button key={c.id} onClick={() => setSelectedCategory(c.id)} className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${selectedCategory === c.id ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}>
                    {c.name}
            ...
            {viewMode === "calendar" && <div className="grid grid-cols-1 lg:grid-cols-[400px,1fr] gap-8">
                {/* Календарь: рамка вровень + sticky на десктопе */}
                <div className="lg:sticky lg:top-24">
                  <div className="inline-block rounded-lg border overflow-hidden shadow-sm px-[8px] py-[8px]">
                    <Calendar mode="single" locale={lv} defaultMonth={defaultMonth} selected={selectedDate} onSelect={date => date && setSelectedDate(date)}
                /* убираем внутренние паддинги обёртки shadcn */ className="!p-0" modifiers={{
                  hasEvent: eventDates
                }} modifiersClassNames={{
                  hasEvent: "relative after:content-[''] after:w-1.5 after:h-1.5 after:rounded-full after:bg-primary after:absolute after:bottom-1.5 after:left-1/2 after:-translate-x-1/2"
                }} />
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <h3 className="text-2xl font-bold">
                    {selectedDate ? `Pasākumi ${selectedDate.toLocaleDateString("lv-LV")}` : "Visi pasākumi"}
                  </h3>
            
                  <div className="space-y-4">
                    {dayFiltered.map(event => <Card key={event.id} className="overflow-hidden hover-lift cursor-pointer group">
            ...
            {viewMode === "list" && <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredEvents.map(event => <Card key={event.id} className="overflow-hidden hover-lift cursor-pointer group">
                    <a href={`/lv/pasakumi/${event.id}/`}>
                      <div className="relative h-48 overflow-hidden">
                        ...
                        <div className="absolute bottom-4 left-4 right-4">
                          <div className="flex items-center justify-between text-white mb-2">
                            ...
                            {event.registrationOpen && <span className="text-xs bg-green-500 px-2 py-1 rounded-full">
                                Pieraksts atvērts
                              </span>}
                          </div>
                        </div>
